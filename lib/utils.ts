...
// Question mapping for consistent ordering
const QUESTION_ID_MAPPING: Record<string, number> = {
  q1: 0,
  q2: 1,
  q3: 2,
  q4: 3,
...
const QUESTION_LABELS = ["Q1", "Q2", "Q3", "Q4", "Q5", "Q6"]
...
function generateGroup(email: string, index: number): string {
  ...
  const groupNum = (index % 3) + 1 // Limit to 3 groups for demo
  return `G${groupNum}`
}
  ...
  if (isTeamData) {
    ...
    return candidates
      .map((candidate, index) => {
        ...
        const teamName = (candidate as any).teamName || `Team ${index + 1}`
...
export function transformApiDataToOverall(
  ...
  const overall: OverallEntry[] = Array.from(map.values()).map((p) => {
    const totalPoints = (p.round1Score || 0) + (p.round2Score || 0) + (p.teamScore || 0) + (p.gameScore || 0)
    return {
      ...p,
